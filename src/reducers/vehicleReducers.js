...
const vehicleReducer=(state=initialState,action)=>{
    switch(action.type){
        case RECEIVE_VEHICLE_LOCATION:
            const vehicleIndex=state.vehicles.vehicles.findIndex(vehicle => vehicle.vehicleID === action.payload.vehicleID)
            if(vehicleIndex!==-1){
                ...
                return{...state,vehicles:[...state.vehicles.vehicles,action.payload]}
            }
        default:
            return state
    }
